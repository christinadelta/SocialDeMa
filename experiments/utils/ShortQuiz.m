function [set] = ShortQuiz(taskNb, set, scrn, keys)
...
while i <= qs 
    ...
    while resp_input == 0 && (GetSecs - startquiz) < resptime
        ...
        elseif ~isempty(pressedKeys) % if subject pressed a valid key
            ...
            if keycode(1,choiceA) % pressed a 
                resp_input  = choiceA;
                rt          = secs - objecton;
                responded   = 1; % a 
                respmade   = secs;
                
            elseif keycode(1,choiceB) % pressed b
                resp_input  = choiceB;
                rt          = secs - objecton;
                responded   = 2; % b
                respmade    = secs;
                
            elseif keycode(1,choiceC) % pressed c
                resp_input  = choiceC;
                rt          = secs - objecton;
                responded   = 3; % c 
                respmade   = secs;
            elseif keycode(1,choiceD) % subject pressed d
                resp_input  = choiceD;
                rt          = secs - objecton;
                responded   = 4; % d
                respmade   = secs;
    ...
    end % end of while response loop
    
    objectoff   = respmade + resptime - ifi;                                  % question-response window self paced or on for 2500 ms
