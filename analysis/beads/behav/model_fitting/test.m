...
for sub = 1:nsubs
    ...
    for cond = 1:conditions 
        ...
        % init condition info struct
        condinfo = [];
        
        % extract sequence and behaviour for current condition
        condsequence        = condseq{1,sub}{1,cond};
        condbehaviour       = condresp{1,sub}{1,cond};
        
        condinfo.condtrials = totaltrials/conditions;
        condinfo.data       = cond_data{1,sub}{1,cond};
        ...
        % model fitting 
        [mparams, lla, aQvec] = bayesbeads(condsequence, condbehaviour, condinfo, alpha, Cw, cost_diff, Cs, cond, sub);
        condlla{sub}{cond} = lla;
        condparams{sub}{cond} = mparams;
        condaQvec{sub}{cond} = aQvec;
