...
for sub = 1:nsubs
    ...
    for block = 1:blocks
        ...
        for trl = 1:blocktrials
            ...
            % run ideal observer 
            [ll, pickTrial, dQvec, ddec, aQvec choice] = estimateLikelihoodf(alpha,Cd,thisq,Cs,this_sequence,1);
            
            pick_trials{sub}{thiscond}{index}    = pickTrial;
            blockdQvec{sub}{thiscond}{index}     = dQvec;
            blockaQvec{sub}{thiscond}{index}     = aQvec;
            blockchoices{sub}{thiscond}{index}   = choice;
            blockddec{sub}{thiscond}{index}      = ddec;
  
        end % end of trial loop
    end % end of block loop
end % end of sub loop

clear thisurn thisq thiscond accurate  aQvec index this_sequence sub ll pickTrial dQvec choice

% remove empty cell arrays. this should result in 40 x 2 x 26 arrays where
% 40 corrsponds to the subjects 2 to the conditions and 26 to the
% trial/index per condition
for sub = 1:nsubs
    for cond = 1:conditions
        pick_trials{1,sub}{1,cond}  = pick_trials{1,sub}{1,cond}(~cellfun('isempty',pick_trials{1,sub}{1,cond}));
        blockdQvec{1,sub}{1,cond}   = blockdQvec{1,sub}{1,cond}(~cellfun('isempty',blockdQvec{1,sub}{1,cond}));
        blockaQvec{1,sub}{1,cond}   = blockaQvec{1,sub}{1,cond}(~cellfun('isempty',blockaQvec{1,sub}{1,cond}));
        blockchoices{1,sub}{1,cond} = blockchoices{1,sub}{1,cond}(~cellfun('isempty',blockchoices{1,sub}{1,cond}));
        blockddec{1,sub}{1,cond}    = blockddec{1,sub}{1,cond}(~cellfun('isempty',blockddec{1,sub}{1,cond}));
    end
    
end

% get mean accuracy, draws and points 
for i = 1:nsubs
    for j = 1:conditions
        
        % compute accuracy 
        tmp_acc             = cell2mat(blockchoices{1,i}{1,j});
        all_accuracy{i}{j}  = mean(tmp_acc == 1);
        
        % compute number of draws
        tmp_draws           = cell2mat(pick_trials{1,i}{1,j});
        all_draws{i}{j}     = mean(tmp_draws);
        
        % compute points
        all_points{i}{j}    = (sum(tmp_acc == 1) * Cc) + (sum(tmp_acc == 2) * Cw) - (sum(tmp_draws) * Cs);
