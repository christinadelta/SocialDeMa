function simoutput = sim_POMDP_Beads_v1(R,params)
...
[~, Qsat, cprob] = mdp_beads(R, drawSequence,params);

% use the choice probabilities to compute choices 
N                               = 1000;
[cprob_samples,model_urnchoice] = modelSamples(cprob,N);

% to allow the model to use one more sample as in the task participants 
% had to at least sample once after the first bead 
% cprob_samples = cprob_samples + 1; 
...
function [ll, Qsat, cprob] = mdp_beads(R, drawSequence,params)
...
for trial = 1:ntrials
    ...
    for draw = 1 : maxDraws
        ...
        Qsad(draw, :) = backWardUtil(thisSequence, draw, maxDraws, R);
        vVec          = Qsad(draw,:);
...
function utility_t = stateUtilBeads(utility, drawi, draw, maxDraws, ng, R)
...
for greenDraws = 0 : futureDraws
    ...
    Qsa = compActionValues(utility, R, ndf, ngf, drawi, maxDraws);
...
function Qsa = compActionValues(utility, R, nd, ng, drawi, maxDraws)
