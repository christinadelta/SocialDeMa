...
% VERSION 2 of formal analysis plan of the best-choice tasks EEG data with SPM12 
% Preprocessing/analysis script was created in December 2022.
% Modified: 16/01/2023
...
outDir      = '/Volumes/DeepSpaceStuff/optimal_stopping_data/economic/spm_analysis/output';
jobsDir     = '/Volumes/DeepSpaceStuff/optimal_stopping_data/economic/spm_analysis/jobs';
cropDir     = '/Volumes/DeepSpaceStuff/optimal_stopping_data/economic/spm_analysis/cropped';
...
% set data path
datadir         = '/Volumes/DeepSpaceStuff/optimal_stopping_data/data/economic/eeg/';  % change to your own data path 
...
for sub = 1:nsubs
    ...
    fprintf('loading economic block data\n')  
...
%         S.save              = 1;
        ...
        if i == 1 % if this is the ERP object
            S.D             = [fullfile(subout, sprintf('eerpfdfMspmeeg_sub_%02d_economic_block_01.mat', sub))
                fullfile(subout, sprintf('eerpfdfMspmeeg_sub_%02d_economic_block_02.mat', sub))
                ];
        else
            S.D             = [fullfile(subout, sprintf('etfrfdfMspmeeg_sub_%02d_economic_block_01.mat', sub))
                fullfile(subout, sprintf('etfrfdfMspmeeg_sub_%02d_economic_block_02.mat', sub))
                ];
        end
...
%  
    ...
    S                       = [];
    S.D                     = fullfile(subout, sprintf('aceerpfdfMspmeeg_sub_%02d_economic_block_01.mat', sub));
    S.robust.ks             = 3;
...
%     the tf file. The ph file can be deleted.
    ...
    S                       = [];
    S.D                     = fullfile(subout, sprintf('tf_cetfrfdfMspmeeg_sub_%02d_economic_block_01.mat', sub));
    S.method                = 'LogR';
    S.prefix                = 'r';
    S.timewin               = [-500 -50];
    S.pooledbaseline        = 0;
    ...
    S                       = [];
    S.D                     = fullfile(subout, sprintf('rtf_cetfrfdfMspmeeg_sub_%02d_economic_block_01.mat', sub));
    S.freqwin               = 13:30;
    S.prefix                = 'P';
    D                       = spm_eeg_avgfreq(S);
...
%     D                       = spm_eeg_avgtime(S);
    ...
    S                       = [];
    S.D                     = fullfile(subout, sprintf('Prtf_cetfrfdfMspmeeg_sub_%02d_economic_block_01.mat', sub));
    S.robust.ks             = 3;
    S.robust.bycondition    = true;
    S.robust.savew          = false;
    S.robust.removebad      = false;
