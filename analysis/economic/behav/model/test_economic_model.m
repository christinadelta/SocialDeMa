%%% Test model fitting for ideal observer mainly
% I created this script to test running the ideal observer model as it
% seems a bit complecated right now
...
for model = 1:Generate_params.num_models   %How many models are we implementing (do_models)?
    ...
    for field = 1:size(fields,1)-1 %exclude name, the last one (total fields should be 15)
...
for model = 1:numel(Generate_params.do_models_identifiers)
    for subI = 1:Generate_params.num_subs
